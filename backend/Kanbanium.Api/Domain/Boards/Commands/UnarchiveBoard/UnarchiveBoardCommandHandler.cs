...
public class UnarchiveBoardCommandHandler : IRequestHandler<UnarchiveBoardCommand, Result>
{
    ...
    private readonly ILogger<UnarchiveBoardCommandHandler> _logger;

    public UnarchiveBoardCommandHandler(
        IApplicationDbContext context,
        ICurrentUserService currentUserService,
        ILogger<UnarchiveBoardCommandHandler> logger)
    {
        _context = context;
    ...
    public async Task<Result> Handle(UnarchiveBoardCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogWarning("User {UserId} attempted to unarchive board {BoardId} without admin rights", userId, request.Id);
            throw new ForbiddenException("Only board admins can unarchive the board");
        }

        board.IsArchived = false;
        await _context.SaveChangesAsync(cancellationToken);

        _logger.LogInformation("User {UserId} unarchived board {BoardId}", userId, request.Id);
