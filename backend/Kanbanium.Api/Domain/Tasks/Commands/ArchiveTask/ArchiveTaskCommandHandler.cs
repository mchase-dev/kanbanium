...
public class ArchiveTaskCommandHandler : IRequestHandler<ArchiveTaskCommand, Result<TaskDto>>
{
    ...
    public async Task<Result<TaskDto>> Handle(ArchiveTaskCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!isMember)
        {
            _logger.LogWarning("User {UserId} attempted to archive task {TaskId} without board membership", userId, request.Id);
        ...
        _logger.LogInformation("User {UserId} archived task {TaskId}", userId, request.Id);
